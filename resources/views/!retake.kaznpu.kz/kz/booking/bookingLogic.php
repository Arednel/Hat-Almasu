...
if (isset($_SESSION['lastRequestTime'])) {
    ...
    if ($diffirenceBetweenTime < 3) {
        header("Location: index.php?error=Келесі өтінімді жіберу үшін,  3 минут күтіңіз");
...
if (isset($_POST["fullName"])) {
    ...
    while ($record = mysqli_fetch_assoc($result)) {
        if ($record['idOfTest'] == $idOfTest && $record['fullName'] == $fullName) {
            header("Location: index.php?error=Сіз өтініш беріп қойғансыз");
    ...
    if (@is_array(getimagesize($_FILES["confirmationFile"]["tmp_name"]))) {
        ...
        if (filesize($uploadedFile) > 8388608) {
            header("Location: index.php?error=Файл тым үлкен");
        ...
        header("Location: index.php?error=Файл қате / Файл тым үлкен");
    ...
    if ($requestID = mysqli_insert_id($conn)) {
        header("Location: index.php?message=Сіздің өтінішіңіз сәтті қабылданды, өтініш нөміріңіз: $requestID. Осы нөмірді сақтаңыз!");

        exit();
    } else {
        header("Location: index.php?error=Қате орын алды");
